...
<div class="dashboard-body">
    <div class="position-relative">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-40 lg-mb-30">
            <h2 class="main-title m0">Modules</h2>
        ...
        <a href="#" class="dash-btn-one Add-Module-Button" type="button" data-bs-toggle="modal" data-bs-target="#Add-Module-Modal"><i class="bi bi-plus"></i>  Skill</a>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id='Add-Module-Form' method="POST" class="clearfix" enctype="multipart/form-data">
...
<div class="modal fade" id="Edit-Module-Modal" tabindex="-1" role="dialog" aria-labelledby="Edit User"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <form id='Edit-Module-Form' method="PUT" class="clearfix" enctype="multipart/form-data">
                    <div class="md-form mb-1" id="edit-alert-error" style="display: none;">
                        <div class="alert alert-dismissable alert-danger shadow-danger">
                            <ul class="list" id="edit-error-message">
                            </ul>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', 'button.Add-Module-Button', function() {
            $('#Add-Module-Form').trigger('reset');
            $('#Add-Module-Form').find($('#error-message')).empty();
            $('#Add-Module-Form').find($('#alert-error')).css('display', 'none');  
        });
        ...
        $('#Add-Module-Form').on('submit', function(e) {
            ...
            table.ajax.url('{{route("modules.index")}}').load().draw();
            } else {
            }
            },
            error: function(res) {
            if (res.responseText) {
            $('#Add-Module-Form').find($('#alert-error')).css('display', 'block');
            ...
            for (let index = 0; index < errors.length; index++) { var html='<li><span>' + errors[index][0] + '</span></li>' ;
                $('#Add-Module-Form').find($('#error-message')).append(html); } $('#alert-error').css('display', 'block' );
                } else { } } }); return false; 
        });
       
        $(document).on('click', 'button.Edit-Module-Button', function() {
            $('#Edit-Module-Form').find($('#edit-error-message')).empty();
            $('#Edit-Module-Form').find($('#edit-alert-error')).css('display', 'none');
            var data = table.row($(this).closest('tr')).data();
            $('#Edit-Module-Form').trigger("reset");
            $('#Edit-Module-Form').find('[name="name"]').val(data.name);
            $('#Edit-Module-Form').find('[name="id"]').val(data.id);
        });
        ...
        $('#Edit-Module-Form').on('submit', function(e) {
            ...
            $('#Edit-Module-Form').find($('#edit-error-message')).empty();
            $('#Edit-Module-Form').find($('#edit-alert-error')).css('display', 'none');
            var SkillId = $('#Edit-Module-Form').find('[name="id"]').val();
            var url = "{{route('modules.update',':id')}}";
            ...
            error: function(res) {
                if (res.responseText) {
                    $('#Edit-Module-Form').find($('#edit-alert-error')).css('display', 'block');
                var errors = JSON.parse(res.responseText).errors;
                var errors = Object.values(errors)
                for (let index = 0; index < errors.length; index++) 
                { 
                    var html='<li><span>' + errors[index][0] + '</span></li>' ;
                    $('#Edit-Module-Form').find($('#edit-error-message')).append(html);
                } 
